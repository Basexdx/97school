...
function Chart({spec}){
  const W=700,H=430,L=85,R=45,T=35,B=75
  ...
  return <div className={styles.chartWrap}><svg viewBox={`0 0 ${W} ${H}`} className={styles.chart} role="img" aria-label={spec.ariaLabel}>{yTicks.map((v,i)=><g key={`y${i}`}><line className={styles.gridLine} x1={L} x2={W-R} y1={Y(v)} y2={Y(v)}/><text className={styles.tickText} x={L-14} y={Y(v)+6} textAnchor="end">{numericText(v)}</text></g>)}{xTicks.map((v,i)=><g key={`x${i}`}><line className={styles.gridLine} x1={X(v)} x2={X(v)} y1={T} y2={H-B}/><text className={styles.tickText} x={X(v)} y={H-B+32} textAnchor="middle">{numericText(v)}</text></g>)}<line className={styles.axis} x1={L} x2={W-R+12} y1={H-B} y2={H-B}/><line className={styles.axis} x1={L} x2={L} y1={H-B} y2={T-10}/>{series.map((s,i)=><g key={i}><polyline className={`${styles.series} ${i===1?styles.series2:''}`} points={s.points.map(([x,y])=>`${X(x)},${Y(y)}`).join(' ')}/>{s.label&&<text className={styles.seriesLabel} x={X(s.points.at(-1)[0])-18} y={Y(s.points.at(-1)[1])-16}>{s.label}</text>}</g>)}<text className={styles.axisLabel} x={W-R} y={H-18} textAnchor="end">{spec.xLabel}</text><text className={styles.axisLabel} x="22" y="28">{spec.yLabel}</text></svg></div>
}
