...
function breakTone(item){return item.id==='winter'||item.id==='february'?'winter':item.id==='spring'?'spring':item.id==='summer'?'summer':'autumn'}
function breakIcon(item){return {winter:'❄',spring:'✿',summer:'☀',autumn:'🍂'}[breakTone(item)]}
...
function ensureMedals(page){
  ...
  topGroup.querySelectorAll('p').forEach((row,index)=>{const badge=row.querySelector('span');if(!badge||index>2)return;badge.classList.add('cg-medal');badge.dataset.place=String(index+1);if(badge.textContent!==String(index+1))badge.textContent=String(index+1)})
}
