...
export default function CosmoUiCleanup(){
  useEffect(()=>{
    ...
    const apply=()=>{
      ...
      for(const sidebar of document.querySelectorAll('.sidebar-light')){
        ...
        for(const button of nav.querySelectorAll('.genius-ref-nav')){
          if(teacher){button.hidden=true;button.setAttribute('aria-hidden','true');button.tabIndex=-1}
          else{button.hidden=false;button.removeAttribute('aria-hidden');button.removeAttribute('tabindex');replaceTrailingText(button,'Постоянные величины','Справочные величины')}
        }
      }
    ...
    const schedule=()=>{if(frame)return;frame=requestAnimationFrame(apply)}
