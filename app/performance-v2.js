...
const holidayStyle=item=>item.id==='winter'||item.id==='february'?'Winter':item.id==='spring'?'Spring':item.id==='summer'?'Summer':'Autumn'
...
function currentSchoolWeek(){
  ...
  return {start:monday.toISOString().slice(0,10),end:sunday.toISOString().slice(0,10),today}
}
...
export function TeacherAcademic(){
  const [classId,setClassId]=useState('class_7A'),[data,setData]=useState(()=>demoAcademic('class_7A')),[selectedLesson,setSelectedLesson]=useState(''),[studentId,setStudentId]=useState(''),[title,setTitle]=useState(''),[description,setDescription]=useState(''),[message,setMessage]=useState('')
  ...
  useEffect(()=>{
    ...
    const ordered=[...data.lessons].sort((a,b)=>a.lesson_date.localeCompare(b.lesson_date))
    const current=ordered.find(l=>l.lesson_date>=week.start&&l.lesson_date<=week.end)
