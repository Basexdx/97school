  ...
  const [rows,breaks]=await Promise.all([
    env.DB.prepare(`
      SELECT l.id AS lesson_id,l.lesson_date,l.topic,l.status AS lesson_status,
        ...
        COALESCE(la.assessment_type,'LESSON') AS assessment_type,
      ...
      FROM lessons l
      LEFT JOIN gradebook_entries g ON g.lesson_id=l.id AND g.student_id=? AND g.kind='LESSON'
      LEFT JOIN lesson_assessments la ON la.lesson_id=l.id
      LEFT JOIN attendance_entries a ON a.lesson_id=l.id AND a.student_id=?
    ...
    `).bind(student.id,student.id,student.id,student.id,student.classId).all(),
    env.DB.prepare(`SELECT id,title,starts_on,ends_on FROM school_breaks WHERE academic_year='2026-2027' ORDER BY starts_on`).all(),
  ])
  const lessons=rows.results||[],today=new Date().toISOString().slice(0,10),next=new Date(Date.now()+86_400_000).toISOString().slice(0,10)
  const reminders=lessons.filter(x=>x.homework_id&&x.homework_due_date===next&&x.homework_status!=='DONE'&&!x.homework_exempt).map(x=>({homeworkId:x.homework_id,title:x.homework_title,lessonDate:x.homework_due_date,message:`Завтра урок. Не забудь выполнить ДЗ: ${x.homework_title}`}))
...
export async function studentRanking(request,env,student){
  ...
  const entries=(result.results||[]).map((row,index)=>{const xp=Number(row.xp||0);if(lastXp===null||xp!==lastXp)rank=index+1;lastXp=xp;return {...row,xp,rank,isCurrent:row.id===student.id}})
...
export async function updateHomeworkProgress(request,env,student,homeworkId){
  const body=await safeJson(request),status=String(body?.status||'')
...
export async function teacherAcademic(request,env,teacher){
  const classId=new URL(request.url).searchParams.get('classId')||'',owned=await ownedClass(env,teacher.id,classId)
  if(!owned)return json({error:'class_not_found'},404)
  const [students,lessons,grades,attendance,homework,overrides]=await Promise.all([
    ...
    env.DB.prepare(`SELECT l.id,l.lesson_date,l.topic,l.status,COALESCE(la.assessment_type,'LESSON') AS assessment_type FROM lessons l LEFT JOIN lesson_assessments la ON la.lesson_id=l.id WHERE l.class_id=? AND l.academic_year='2026-2027' ORDER BY l.lesson_date`).bind(classId).all(),
    ...
    env.DB.prepare(`SELECT a.lesson_id,a.student_id,a.status FROM attendance_entries a JOIN lessons l ON l.id=a.lesson_id WHERE l.class_id=?`).bind(classId).all(),
  ...
  return json({class:owned,students:students.results||[],lessons:lessons.results||[],grades:grades.results||[],attendance:attendance.results||[],homework:homework.results||[],overrides:overrides.results||[]})
}

export async function saveGrade(request,env,teacher){
  const body=await safeJson(request),lessonId=String(body?.lessonId||''),studentId=String(body?.studentId||''),value=Number(body?.value),kind=['LESSON','HOMEWORK','TEST','LAB'].includes(body?.kind)?body.kind:'LESSON'
...
export async function saveAttendance(request,env,teacher){
  const body=await safeJson(request),lessonId=String(body?.lessonId||''),studentId=String(body?.studentId||''),status=String(body?.status||'')
  if(!['ABSENT','PRESENT'].includes(status))return json({error:'invalid_attendance_status'},400)
  const row=await env.DB.prepare(`SELECT l.id FROM lessons l JOIN classes c ON c.id=l.class_id JOIN students s ON s.class_id=c.id WHERE l.id=? AND s.id=? AND c.teacher_id=?`).bind(lessonId,studentId,teacher.id).first()
  if(!row)return json({error:'lesson_or_student_not_found'},404)
  if(status==='PRESENT'){
    await env.DB.prepare(`DELETE FROM attendance_entries WHERE lesson_id=? AND student_id=?`).bind(lessonId,studentId).run()
...
export async function saveLessonAssessment(request,env,teacher){
  const body=await safeJson(request),lessonId=String(body?.lessonId||''),assessmentType=String(body?.assessmentType||'')
  if(!['LESSON','INDEPENDENT','TEST','HOMEWORK'].includes(assessmentType))return json({error:'invalid_assessment_type'},400)
  const row=await env.DB.prepare(`SELECT l.id FROM lessons l JOIN classes c ON c.id=l.class_id WHERE l.id=? AND c.teacher_id=?`).bind(lessonId,teacher.id).first()
  if(!row)return json({error:'lesson_not_found'},404)
  await env.DB.prepare(`INSERT INTO lesson_assessments(lesson_id,assessment_type,updated_by) VALUES(?,?,?) ON CONFLICT(lesson_id) DO UPDATE SET assessment_type=excluded.assessment_type,updated_by=excluded.updated_by,updated_at=CURRENT_TIMESTAMP`).bind(lessonId,assessmentType,teacher.id).run()
  return json({lessonId,assessmentType},201)
}

export async function saveHomework(request,env,teacher){
  const body=await safeJson(request),classId=String(body?.classId||''),lessonId=String(body?.lessonId||''),title=clean(body?.title,120),description=clean(body?.description,2000)
  ...
  await env.DB.prepare(`INSERT INTO homework(id,class_id,lesson_id,title,description,created_by) VALUES(?,?,?,?,?,?) ON CONFLICT(class_id,lesson_id) DO UPDATE SET title=excluded.title,description=excluded.description,updated_at=CURRENT_TIMESTAMP`).bind(id,classId,lessonId,title,description,teacher.id).run()
...
export async function saveHomeworkOverride(request,env,teacher,homeworkId){
  ...
  const title=body?.title==null?null:clean(body.title,120),description=body?.description==null?null:clean(body.description,2000),isExempt=body?.isExempt?1:0,id=crypto.randomUUID()
  await env.DB.prepare(`INSERT INTO homework_overrides(id,homework_id,student_id,title,description,lesson_id,is_exempt,updated_by) VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(homework_id,student_id) DO UPDATE SET title=excluded.title,description=excluded.description,lesson_id=excluded.lesson_id,is_exempt=excluded.is_exempt,updated_by=excluded.updated_by,updated_at=CURRENT_TIMESTAMP`).bind(id,homeworkId,studentId,title,description,lessonId,isExempt,teacher.id).run()
